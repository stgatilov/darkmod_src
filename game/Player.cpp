...
bool idPlayer::SelectWeapon( int num, bool force )
{
	...
	for (int i = 0; i < category->GetNumItems(); i++)
	{
		...
		if (item != NULL && item->GetWeaponIndex() == num)
		{
			...
			{
				m_dynamicHUD.weapon.Show(true); // #6677: DynHUD_WeaponRule2
			}
 ...
 */
		...
		*/
		...
		case IMPULSE_INVENTORY_USE:	// Inventory use item
		{
			...
			m_dynamicHUD.inventory.Show(true); // #6677: DynHUD_InventoryRule5
...
void idPlayer::DynamicHudT::Update()
{
	...
	else
	{
		...
		// #6677: DynHUD_InventoryRule4
		{
			...
			{
				if (inventory.ShouldBeHiddenIndefinitely())
				{
					useOnFrobItem = item;
		...
		{
			if (health.ShouldBeHiddenIndefinitely())
			{
...
bool idPlayer::FrobHandling::TryUseOnFrob(EButtonState state, idEntity* target)
{
	...
	if (frobAction == EFrobAction::UseOnFrobInit)
	{
		...
		if (cv_tdm_inv_use_visual_feedback.GetBool())
		{
			m_player->m_dynamicHUD.inventory.Show(true); // #6677: DynHUD_InventoryRule5			

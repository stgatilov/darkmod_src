...
void main() {
	...
	R1 = texture(u_texture4, var_tc7.xy);                                                               //TEX	R1, fragment.texcoord[7].xyxx, texture[4], 2D;
	R1.x = localNormal.a;                                                                               //MOV	R1.x, localNormal.a;				
	R1 = (R1) * (scaleTwo) + (subOne);                                                                  //MAD	R1, R1, scaleTwo, subOne;
	
	// perform vertex blend
	localNormal = mix(R1, localNormal, var_color);                                                      //LRP	localNormal, fragment.color, localNormal, R1;
	...
	R1 = texture(u_texture5, var_tc6.zw);                                                               //TEX	R1, fragment.texcoord[6].zwxx, texture[5], 2D;
	
	// perform vertex blend
	colDiffuse = mix(R1, colDiffuse, var_color);                                                        //LRP	colDiffuse, fragment.color, colDiffuse, R1;
	
	// Load Specular maps
	colSpecular = texture(u_texture3, var_tc6.xy);                                                      //TEX	colSpecular, fragment.texcoord[6].xyxx, texture[3], 2D;
	R1 = texture(u_texture6, var_tc7.zw);                                                               //TEX	R1, fragment.texcoord[7].zwxx, texture[6], 2D;
	
	// perform vertex blend
	colSpecular = mix(R1, colSpecular, var_color);                                                      //LRP	colSpecular, fragment.color, colSpecular, R1;

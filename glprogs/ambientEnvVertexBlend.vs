...
void main() {
	...
	var_tc6.xy = (attr_TexCoord.xy) * (u_localParam1.xy);                                               //MUL	result.texcoord[6].xy, vertex.attrib[8], program.local[1];
	var_tc6.zw = (attr_TexCoord.xx) * (u_localParam1.xy);                                               //MUL	result.texcoord[6].zw, vertex.attrib[8].xxxy, program.local[1];
	
	// texcoord 7 takes the scaled texture coordinates for bumpmap2 and specularmap2 respectively.
	var_tc7.xy = (attr_TexCoord.xy) * (u_localParam2.xy);                                               //MUL	result.texcoord[7].xy, vertex.attrib[8], program.local[2];
	var_tc7.zw = (attr_TexCoord.xx) * (u_localParam2.xy);                                               //MUL	result.texcoord[7].zw, vertex.attrib[8].xxxy, program.local[2];
	
	//texCoord 5 gets ambient light color.
	var_tc5 = u_localParam3;                                                                            //MOV	result.texcoord[5], program.local[3];

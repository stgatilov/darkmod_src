...
void main() {
	...
	vec4 diffuse, specular, gloss, color, ambient, dirSky;                                              //TEMP	diffuse, specular, gloss, color, ambient, dirSky; 
	...
	R1.w = 1.0 / R1.w;                                                                                  //RCP 	R1.w, R1.w;
	R1.w = clamp((0.8) + (-R1.w), 0.0, 1.0);                                                            //ADD_SAT R1.w, 0.8, -R1.w;
	R1.w = (R1.w) * (R1.w);                                                                             //MUL 	R1.w, R1.w, R1.w;
	
	atten = (lightCube) * (R1.wwww);                                                                    //MUL 	atten, lightCube, R1.w;
	atten = (atten) * (vec4(0.4));                                                                      //MUL     atten, atten, 0.4;
	
	// early out - attenuation
	atten.w = dot(atten.xyz, myconst.xxx);                                                              //DP3 	atten.w, atten, const.x;
	atten.w = float((0.0) >= (atten.w));                                                                //SGE 	atten.w, 0.0, atten.w;
	if ((-atten.w) < 0.0) discard;                                                                      //KIL -atten.w;

...
void main() {
	...
	//-------------------------------------------
	//See if this light is an ambient light
	//-------------------------------------------
	R1 = texture(u_texture0, cubemapLookupVec.xyz);                                                     //TEX	R1, cubemapLookupVec, texture[0], CUBE;
	R2 = texture(u_texture0, -cubemapLookupVec.xyz);                                                    //TEX	R2, -cubemapLookupVec, texture[0], CUBE;
	
	ambientLightParams.z = (R2.x) - (R1.x);                                                             //SUB	ambientLightParams.z, R2.x, R1.x;
	ambientLightParams.y = mix(1, 0, step(float(0.0), ambientLightParams.z));                           //CMP ambientLightParams.y, ambientLightParams.z, 1, 0;		
	ambientLightParams.x = (1) - (ambientLightParams.y);                                                //SUB ambientLightParams.x, 1, ambientLightParams.y;			
	
	//Ambient lighting test: Always kill directional lighting.
	//MOV		ambientLightParams.y, 0;
	//-------------------------------------------
	...
	//---------------------------------------------------------
	ambientContrib = vec4(dot(localNormal.xyz, var_tc7.xyz));                                           //DP3		ambientContrib, localNormal, fragment.texcoord[7];
	...
	ambientContrib = (ambientContrib) * (ambientLightParams.xxxx);                                      //MUL		ambientContrib, ambientContrib, ambientLightParams.x;
	//---------------------------------------------------------
	light = (ambientLightParams.yyyy) * (light) + (ambientContrib);                                     //MAD	light, ambientLightParams.y, light, ambientContrib;  
	...
	//MAD		R3, R2, .5, .5;
	R1 = mix(lightParms.yyyy, lightParms.xxxx, step(vec4(0.0), ambientLightParams.zzzz));               //CMP		R1, ambientLightParams.z, lightParms.y, lightParms.x;
	
	rimLight = (rimLight) * (R1);                                                                       //MUL		rimLight, rimLight, R1;
	...
	R1 = (R1) * (ambientLightParams.yyyy);                                                              //MUL		R1, R1, ambientLightParams.y;		
	//---------------------------------------------------------
	...
	R2 = max(R2, ambientLightParams.xxxx);                                                              //MAX			R2, R2, ambientLightParams.x;						
	light = (rimLight) * (R2) + (light);                                                                //MAD			light, rimLight, R2, light;
	//MAD			light, R1, R2, light;

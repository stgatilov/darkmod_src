...
void main() {
	...
	R1.w = clamp((0.82) + (-R1.w), 0.0, 1.0);                                                           //ADD_SAT R1.w, 0.82, -R1.w;
	R1.w = (R1.w) * (R1.w);                                                                             //MUL 	R1.w, R1.w, R1.w;
	
	atten = (lightCube) * (R1.wwww);                                                                    //MUL 	atten, lightCube, R1.w;
	atten = (atten) * (vec4(0.5));                                                                      //MUL     atten, atten, 0.5;
	...
	specular = clamp((light) - (vec4(.3)), 0.0, 1.0);                                                   //SUB_SAT	specular, light, .3;
	specular = min(specular, fresnelParms.zzzz);                                                        //MIN		specular, specular, fresnelParms.zzzz;
	rimLight = (specular) * (fresnelTerm);                                                              //MUL		rimLight, specular, fresnelTerm;

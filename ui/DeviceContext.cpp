...
// fva/DG: added adjustCoords argument for CstDoom3 anchored GUIs, it basically
//         replaces calling AdjustCoords(&x, &y, &w, &h);  before calling this
void idDeviceContext::DrawStretchPic(float x, float y, float w, float h, float s1, float t1, float s2, float t2, const idMaterial *shader, bool adjustCoords) {
	...
	if (adjustCoords) {
		for( int i=0; i<4; ++i) {
			...
			verts[i].xyz[0] = verts[i].xyz[0] * xScale + cst_xOffset;
			verts[i].xyz[1] = verts[i].xyz[1] * yScale + cst_yOffset;
		}
	}
	//#modified-fva; END
...
void idDeviceContext::DrawCursor(float *x, float *y, float size) {
	...
	//     It had to be adjusted for the CstDoom3 anchored GUIs, so all that is now done here
	...
	float sizeW = size * xScale;
	float sizeH = size * yScale;
	float fixedX = *x;
	float fixedY = *y;
